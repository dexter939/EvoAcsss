...
- **Multi-Tenant Device Access Control**: Role-based device scoping via `user_devices` pivot table with three permission levels (viewer, manager, admin), enforced by `EnsureDeviceAccess` middleware. **Phase 0-4 Complete (December 2025)**: Tenant/TenantCredential models, TenantDiscoveryService (subdomain/header/token/user resolution), TenantContext singleton, HasTenant trait for auto-scoping, TenantAwareTokenService for Sanctum token scoping, TenantScope global scope, IdentifyTenant/EnforceTenantContext middleware, ValidateTokenTenant middleware for token validation, TenantDatabaseSessionHandler for session isolation, RotateTenantSecrets command for secret rotation, TenantAnomalyDetector for threat detection, SecurityAlertService for multi-channel alerting. **Legacy Fallback Removed (December 2025)**: All backward compatibility code removed, tenant_id now required (NOT NULL) on users/devices/alarms/sessions/tokens, tenant isolation enforced by default. See `docs/LEGACY_FALLBACK_REMOVAL.md` for migration guide. **BBF.369 Compliance**: Full compliance verified - see `docs/BBF369_COMPLIANCE_VERIFICATION.md`.
- **Real-Time Broadcasting (Laravel Reverb v1.6.0)**: WebSocket server for real-time alarm notifications with carrier-grade multi-tenant isolation. Broadcasts exclusively to tenant channels (`tenant.{id}`, `tenant.{id}.alarms.{severity}`) for full tenant-based coverage. All users within a tenant automatically receive alarms for their tenant's devices.
